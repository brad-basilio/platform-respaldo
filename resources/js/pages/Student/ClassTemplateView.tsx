        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                <div className="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-cyan-500/10 to-transparent">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-cyan-600 rounded-lg flex items-center justify-center">
                      <Zap className="w-5 h-5 text-white" />
                    </div>
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900">Sesiones de Práctica</h3>
                      <p className="text-sm text-gray-500">Refuerza lo aprendido</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6 space-y-4">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-bold text-cyan-600">
                      {practiceEnrollments.filter(e => e.scheduled_class.status === 'completed').length} / {practiceSettings.min_required} mín.
                    </span>
                  </div>
                  
                  <Progress 
                    value={Math.min(100, (practiceEnrollments.filter(e => e.scheduled_class.status === 'completed').length / practiceSettings.min_required) * 100)} 
                    className="h-2 bg-cyan-100 [&>div]:bg-cyan-500"
                  />

                  {/* List of practices */}
                  <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
                    {practiceEnrollments.map((pe) => (
                      <div key={pe.id} className="p-3 rounded-lg border border-cyan-100 bg-cyan-50/30">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <Calendar className="w-3.5 h-3.5 text-cyan-600" />
                            <p className="text-xs font-semibold text-gray-800 capitalize">
                              {new Date(pe.scheduled_class.scheduled_at).toLocaleDateString('es-PE', { weekday: 'short', day: 'numeric', month: 'short' })}
                            </p>
                          </div>
                          <Badge className={`${pe.scheduled_class.status === 'completed' ? 'bg-emerald-500' : 'bg-[#073372]'} text-white border-0 text-[10px] px-1.5 h-5`}>
                            {pe.scheduled_class.status === 'completed' ? 'Completada' : 'Programada'}
                          </Badge>
                        </div>
                        ...
                        <div className="flex items-center justify-between text-[11px] text-gray-500 mb-2">
                          <span className="flex items-center gap-1">
                            ...
                            {new Date(pe.scheduled_class.scheduled_at).toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit' })}
                          </span>
                          {pe.scheduled_class.teacher && (
                            <span className="flex items-center gap-1">
                              <Users className="w-3 h-3" />
                              {pe.scheduled_class.teacher.name}
                            </span>
                          )}
                        </div>

                        {pe.scheduled_class.status === 'scheduled' && pe.scheduled_class.meet_url && (
                          <Button 
                            className="w-full bg-[#17BC91] hover:bg-[#14a77f] text-white h-8 text-xs font-bold shadow-sm"
                            onClick={() => window.open(pe.scheduled_class.meet_url, '_blank')}
                          >
                  ...
                  {!isClassCompleted ? (
                    ...
                    <div className="flex items-start gap-2 p-3 bg-emerald-50 rounded-lg border border-emerald-200">
                      ...
                      <p className="text-xs text-emerald-700">
                        Has completado el límite máximo de prácticas para este módulo. ¡Excelente trabajo!
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      <p className="text-[11px] text-gray-500 italic">
                        * Máximo 1 práctica por día.
                      </p>
                      <Button
                        className="w-full bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-5 shadow-lg shadow-cyan-600/20"
                        onClick={() => {
                          setRequestType('practice');
                          setShowRequestModal(true);
                        }}
                      >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-white">
                    {requestType === 'practice' ? 'Solicitar sesión de práctica' : 'Solicitar Clase'}
                  </h3>
